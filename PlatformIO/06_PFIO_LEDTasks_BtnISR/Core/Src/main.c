...
TaskHandle_t tsk_zLEDGreenTskHdl;
TaskHandle_t tsk_zLEDOrangeTskHdl;
TaskHandle_t tsk_zLEDRedTskHdl;
...
int main(void)
{
  ...
  /*Create Tasks*/
  status = xTaskCreate( led_GreenHandler, "LED Green Task", 200, NULL, 3, &tsk_zLEDGreenTskHdl );
  configASSERT( status == pdPASS );
  status = xTaskCreate( led_RedHandler, "LED Red Task", 200, NULL, 1, &tsk_zLEDRedTskHdl );
  configASSERT( status == pdPASS );
...
static void led_GreenHandler( void *params )
{
  ...
  while( 1 )
  {
    ...
    isNotified = xTaskNotifyWait( 0, 0, NULL, pdMS_TO_TICKS( 1000 ) );
...
static void led_OrangeHandler( void *params )
{
  ...
  while( 1 )
  {
    ...
    /*Wait for notification for 1 second*/
    isNotified = xTaskNotifyWait( 0, 0, NULL, pdMS_TO_TICKS( 800 ) );
...
static void led_RedHandler( void *params )
{
  ...
  while( 1 )
  {
    ...
    /*Wait for notification for 1 second*/
    isNotified = xTaskNotifyWait( 0, 0, NULL, pdMS_TO_TICKS( 400 ) );
